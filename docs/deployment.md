...
Build with `pnpm build`, apply schema changes with `pnpm db:migrate`, and start with `pnpm start`. Configure load balancers to use `/api/health` for liveness and `/api/health/ready` for readiness. Do not gate global readiness on GitHub, Google, Docker, or Playwright availability.
