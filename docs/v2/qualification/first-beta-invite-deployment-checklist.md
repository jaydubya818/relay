...
Status: **NO-GO** until the exact release revision, public Relay and Builder origins, production backup, and fresh-owner acceptance are verified. Current Relay main contains `drizzle/0024_beta_invites.sql`, the owner invitation UI, and invite-only signup. [PR #24](https://github.com/jaydubya818/relay/pull/24) keeps the one-use token out of query strings. The old UI-audit worktree has a different `0024_tester_invitations.sql` design and must not be used as this release source.
...
- A bounded read-only production query succeeded. Exactly 24 stored migration hashes and timestamps match current Relay main through `0023_channel_execution`; `beta_invites` is absent. Baseline counts at audit were 3 accounts, 3 users, and 2 agents. A disposable local database received those same 24 SQL migrations, then `0024_beta_invites` applied cleanly as migration 25, creating the invitation table with account/user counts unchanged. This proves the journal lineage and additive migration path, not that production has no manual schema drift. The temporary production environment file and rehearsal database were deleted.
...
- The Builder alias `https://myeve-builder-jaydubya818.vercel.app` likewise redirects an unauthenticated `/api/template-version` request to Vercel SSO. Its Ready deployment `dpl_H3U7KPbdFY7pNih4UK9N9V24LKmq` is not a verified public signup destination, and its source SHA was not established by deployment metadata.
- Recommended access change, subject to the owner's production approval: set both projects to [Vercel Standard Protection](https://vercel.com/docs/deployment-protection) so production domains are publicly reachable while preview deployments remain protected. Keep Relay's own authentication and invitation-only signup. Verify both exact origins without a Vercel session before sending any link. Restore the prior protection scope if verification fails.
- Relay main at `221a9e7` contains the invitation implementation. On the isolated PR #24 candidate, local auth tests (7/7), full CI (437 passed, 6 skipped), typecheck, lint, migration consistency, build, and a synthetic clean-browser signup/replay check passed. Relay PR #20 was closed without merging and is separate Factory work.
