  ...
  return (
    <>
      ...
      <form className="filters card">
        ...
        <div className="field"><label>Provider</label><select name="provider" defaultValue={params.provider ?? ""}><option value="">All providers</option><option>GITHUB</option></select></div>
        <button className="button">Filter</button>
      </form>
      <div className="card flush table-wrap">
        <table><thead><tr><th>Time</th><th>Agent</th><th>Capability</th><th>Provider</th><th>Status</th><th>Latency</th><th>Request</th></tr></thead><tbody>{activity.map((item) => <tr key={item.id}><td className="subtle">{new Date(item.createdAt).toLocaleString()}</td><td>{item.agentName ?? "System"}</td><td className="mono">{item.capability}</td><td>{item.provider ?? "Relay"}</td><td><Status value={item.status} /></td><td>{item.durationMs} ms</td><td className="mono subtle">{item.sessionId.slice(0, 8)}</td></tr>)}</tbody></table>
