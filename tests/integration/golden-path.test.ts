...
describe("Relay V0 golden path", () => {
  ...
  beforeEach(() => {
    accountId = freshDatabase().accountId;
  ...
  afterEach(() => { vi.unstubAllGlobals(); cleanupDatabase(); });

  it("shares authorized memory across agents while private memory remains isolated", async () => {
    const agentA = createAgent(accountId, { name: "Claude Test Agent", capabilities: ["memory.read", "memory.write"] });
    const agentB = createAgent(accountId, { name: "Codex Test Agent", capabilities: ["memory.read"] });
  ...
  it("uses one GitHub connection with per-agent authorization and auditable denial", async () => {
    const agentA = createAgent(accountId, { name: "Claude Test Agent", capabilities: ["github.repo.read"] });
    const agentB = createAgent(accountId, { name: "Codex Test Agent", capabilities: ["memory.read"] });
    ...
    setCapabilityGrant(accountId, agentB.agentId, "github.repo.read", "ALLOW");
    expect(JSON.stringify(await call(agentB.credential, "relay_github_repo_list"))).toContain("relay-test/relay");
    const statuses = (db().prepare("SELECT status FROM activities WHERE account_id = ? AND capability = 'github.repo.read' ORDER BY rowid").all(accountId) as any[]).map((row) => row.status);
    expect(statuses).toEqual(["SUCCESS", "DENIED", "SUCCESS"]);
  });

  it("revokes a credential immediately without a process restart", async () => {
    const agent = createAgent(accountId, { name: "Revocation Agent", capabilities: ["memory.read"] });
    await expect(handleMcp(agent.credential, { jsonrpc: "2.0", id: 1, method: "tools/list" })).resolves.toBeDefined();
    revokeCredential(accountId, agent.agentId);
