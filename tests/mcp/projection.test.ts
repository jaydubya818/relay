...
describe("MCP tool projection", () => {
  ...
  it("only projects tools backed by allowed grants", async () => {
    const agent = await createAgent(accountId, { name: "Scoped Agent", capabilities: ["memory.read"] });
    ...
    await setCapabilityGrant(accountId, agent.agentId, "github.repo.read", "ALLOW");
