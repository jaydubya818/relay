...
export const connectorProvider = pgEnum("connector_provider", ["GOOGLE_DRIVE", "LINEAR"]);
export const connectorOperationStatus = pgEnum("connector_operation_status", ["PREPARED", "RUNNING", "SUCCEEDED", "FAILED", "EFFECT_UNKNOWN"]);
export const financialAccountStatus = pgEnum("financial_account_status", ["ACTIVE", "DISCONNECTED", "STALE"]);
export const paymentCredentialStatus = pgEnum("payment_credential_status", ["ACTIVE", "REVOKED"]);
export const purchaseIntentStatus = pgEnum("purchase_intent_status", ["PENDING_APPROVAL", "READY_FOR_CHECKOUT", "HUMAN_CHECKOUT", "COMMIT_READY", "REVIEW_REQUIRED", "COMPLETED", "EFFECT_UNKNOWN", "CANCELLED"]);
export const checkoutPermitStatus = pgEnum("checkout_permit_status", ["ACTIVE", "COMMIT_READY", "CONSUMED", "REVOKED"]);
export const delegationStatus = pgEnum("delegation_status", ["ACTIVE", "COMPLETED", "FAILED", "REVOKED"]);
export const delegatedAuthorityStatus = pgEnum("delegated_authority_status", ["AVAILABLE", "CLAIMED", "REVOKED"]);
...
export const connectorDefinitions = pgTable("connector_definitions", {
  id: text("id").primaryKey(), provider: connectorProvider("provider").notNull(), version: text("version").notNull(), manifest: jsonb("manifest").notNull(), manifestHash: text("manifest_hash").notNull(), signature: text("signature").notNull(), signingKeyId: text("signing_key_id").notNull(), status: providerDefinitionStatus("status").notNull().default("ACTIVE"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("connector_definition_version_idx").on(table.provider, table.version), uniqueIndex("connector_definition_hash_idx").on(table.manifestHash)]);

export const connectorConnections = pgTable("connector_connections", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), definitionId: text("definition_id").notNull().references(() => connectorDefinitions.id, { onDelete: "restrict" }), provider: connectorProvider("provider").notNull(), externalAccountId: text("external_account_id").notNull(), displayName: text("display_name").notNull(), scopes: text("scopes").array().notNull(), restrictions: jsonb("restrictions").notNull(), credentialHandle: text("credential_handle").notNull(), permissionSnapshot: jsonb("permission_snapshot").notNull(), permissionSnapshotHash: text("permission_snapshot_hash").notNull(), status: connectionStatus("status").notNull().default("CONNECTED"), lastVerifiedAt: timestamp("last_verified_at", { withTimezone: true, mode: "string" }).notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), revokedAt: timestamp("revoked_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("connector_connection_provider_idx").on(table.accountId, table.provider), index("connector_connection_account_idx").on(table.accountId, table.status)]);

export const connectorOauthFlows = pgTable("connector_oauth_flows", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), principalId: text("principal_id").notNull().references(() => principals.id, { onDelete: "cascade" }), provider: connectorProvider("provider").notNull(), stateHash: text("state_hash").notNull(), codeVerifierHandle: text("code_verifier_handle").notNull(), redirectUri: text("redirect_uri").notNull(), requestedScopes: text("requested_scopes").array().notNull(), restrictions: jsonb("restrictions").notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), expiresAt: timestamp("expires_at", { withTimezone: true, mode: "string" }).notNull(), consumedAt: timestamp("consumed_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("connector_oauth_state_idx").on(table.stateHash), index("connector_oauth_account_idx").on(table.accountId, table.provider, table.expiresAt)]);

export const connectorResources = pgTable("connector_resources", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), connectionId: text("connection_id").notNull().references(() => connectorConnections.id, { onDelete: "cascade" }), providerType: text("provider_type").notNull(), externalId: text("external_id").notNull(), containerExternalId: text("container_external_id").notNull(), metadata: jsonb("metadata").notNull().default({}), permissionsHash: text("permissions_hash").notNull(), lastVerifiedAt: timestamp("last_verified_at", { withTimezone: true, mode: "string" }).notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("connector_resource_external_idx").on(table.connectionId, table.providerType, table.externalId), index("connector_resource_account_idx").on(table.accountId, table.connectionId)]);

export const connectorOperations = pgTable("connector_operations", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), connectionId: text("connection_id").notNull().references(() => connectorConnections.id, { onDelete: "restrict" }), taskId: text("task_id").notNull(), actionIntentId: text("action_intent_id").notNull(), leaseId: text("lease_id").notNull(), capability: text("capability").notNull(), resourceHash: text("resource_hash").notNull(), parameters: jsonb("parameters").notNull(), parametersHash: text("parameters_hash").notNull(), idempotencyKey: text("idempotency_key").notNull(), providerResourceId: text("provider_resource_id"), status: connectorOperationStatus("status").notNull().default("PREPARED"), attemptCount: integer("attempt_count").notNull().default(0), receipt: jsonb("receipt"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("connector_operation_idempotency_idx").on(table.accountId, table.idempotencyKey), index("connector_operation_account_idx").on(table.accountId, table.connectionId, table.status)]);

export const financialAccounts = pgTable("financial_accounts", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), provider: text("provider").notNull(), externalAccountId: text("external_account_id").notNull(), displayName: text("display_name").notNull(), kind: text("kind").notNull(), currency: text("currency").notNull(), balance: numeric("balance", { precision: 30, scale: 9 }), balanceAsOf: timestamp("balance_as_of", { withTimezone: true, mode: "string" }), status: financialAccountStatus("status").notNull().default("ACTIVE"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("financial_account_external_idx").on(table.accountId, table.provider, table.externalAccountId), index("financial_account_tenant_idx").on(table.accountId, table.status)]);

export const financialTransactions = pgTable("financial_transactions", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), financialAccountId: text("financial_account_id").notNull().references(() => financialAccounts.id, { onDelete: "cascade" }), providerTransactionId: text("provider_transaction_id").notNull(), amount: numeric("amount", { precision: 30, scale: 9 }).notNull(), currency: text("currency").notNull(), merchant: text("merchant"), description: text("description"), occurredAt: timestamp("occurred_at", { withTimezone: true, mode: "string" }).notNull(), providerData: jsonb("provider_data").notNull().default({}), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("financial_transaction_external_idx").on(table.financialAccountId, table.providerTransactionId), index("financial_transaction_tenant_idx").on(table.accountId, table.occurredAt)]);

export const paymentCredentialReferences = pgTable("payment_credential_references", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), provider: text("provider").notNull(), credentialHandle: text("credential_handle").notNull(), displayLabel: text("display_label").notNull(), status: paymentCredentialStatus("status").notNull().default("ACTIVE"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), revokedAt: timestamp("revoked_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("payment_credential_handle_idx").on(table.accountId, table.credentialHandle), index("payment_credential_tenant_idx").on(table.accountId, table.status)]);

export const purchaseIntents = pgTable("purchase_intents", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), agentId: text("agent_id").notNull().references(() => agents.id, { onDelete: "restrict" }), runtimeClientId: text("runtime_client_id").notNull().references(() => runtimeClients.id, { onDelete: "restrict" }), taskId: text("task_id").notNull().references(() => v2Tasks.id, { onDelete: "restrict" }), actionIntentId: text("action_intent_id").notNull(), actionHash: text("action_hash").notNull(), actionSnapshot: jsonb("action_snapshot").notNull(), merchantId: text("merchant_id").notNull(), merchantName: text("merchant_name").notNull(), currency: text("currency").notNull(), requestedAmount: numeric("requested_amount", { precision: 30, scale: 9 }).notNull(), taxShippingTolerance: numeric("tax_shipping_tolerance", { precision: 30, scale: 9 }).notNull().default("0"), authorizedMaximum: numeric("authorized_maximum", { precision: 30, scale: 9 }).notNull(), items: jsonb("items").notNull(), credentialReferenceId: text("credential_reference_id").notNull().references(() => paymentCredentialReferences.id, { onDelete: "restrict" }), budgetReservationId: text("budget_reservation_id").notNull().references(() => budgetReservations.id, { onDelete: "restrict" }), approvalRequestId: text("approval_request_id").references(() => approvalRequests.id, { onDelete: "restrict" }), approvalDecisionId: text("approval_decision_id").references(() => approvalDecisions.id, { onDelete: "restrict" }), controlSessionId: text("control_session_id").references(() => computerControlSessions.id, { onDelete: "restrict" }), status: purchaseIntentStatus("status").notNull().default("PENDING_APPROVAL"), idempotencyKey: text("idempotency_key").notNull(), finalAmount: numeric("final_amount", { precision: 30, scale: 9 }), providerReference: text("provider_reference"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), completedAt: timestamp("completed_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("purchase_intent_idempotency_idx").on(table.accountId, table.idempotencyKey), uniqueIndex("purchase_intent_action_idx").on(table.accountId, table.actionIntentId), index("purchase_intent_tenant_status_idx").on(table.accountId, table.status, table.createdAt)]);

export const purchaseCheckoutPermits = pgTable("purchase_checkout_permits", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), purchaseIntentId: text("purchase_intent_id").notNull().references(() => purchaseIntents.id, { onDelete: "cascade" }), principalId: text("principal_id").notNull().references(() => principals.id, { onDelete: "restrict" }), controlSessionId: text("control_session_id").notNull().references(() => computerControlSessions.id, { onDelete: "restrict" }), fenceToken: bigint("fence_token", { mode: "number" }).notNull(), tokenHash: text("token_hash").notNull(), status: checkoutPermitStatus("status").notNull().default("ACTIVE"), expiresAt: timestamp("expires_at", { withTimezone: true, mode: "string" }).notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), consumedAt: timestamp("consumed_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("purchase_checkout_token_idx").on(table.tokenHash), uniqueIndex("purchase_checkout_active_idx").on(table.accountId, table.purchaseIntentId), index("purchase_checkout_tenant_idx").on(table.accountId, table.expiresAt)]);

export const purchaseReceipts = pgTable("purchase_receipts", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), purchaseIntentId: text("purchase_intent_id").notNull().references(() => purchaseIntents.id, { onDelete: "restrict" }), providerReference: text("provider_reference").notNull(), merchantId: text("merchant_id").notNull(), merchantName: text("merchant_name").notNull(), amount: numeric("amount", { precision: 30, scale: 9 }).notNull(), currency: text("currency").notNull(), receiptData: jsonb("receipt_data").notNull(), receiptHash: text("receipt_hash").notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("purchase_receipt_intent_idx").on(table.accountId, table.purchaseIntentId), uniqueIndex("purchase_receipt_provider_idx").on(table.accountId, table.providerReference), index("purchase_receipt_tenant_idx").on(table.accountId, table.createdAt)]);

export const agentDelegations = pgTable("agent_delegations", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), parentDelegationId: text("parent_delegation_id"), parentTaskId: text("parent_task_id").notNull().references(() => v2Tasks.id, { onDelete: "restrict" }), childTaskId: text("child_task_id").notNull().references(() => v2Tasks.id, { onDelete: "restrict" }), parentAgentId: text("parent_agent_id").notNull().references(() => agents.id, { onDelete: "restrict" }), childAgentId: text("child_agent_id").notNull().references(() => agents.id, { onDelete: "restrict" }), authorizationLeaseId: text("authorization_lease_id").notNull().references(() => capabilityLeases.id, { onDelete: "restrict" }), objective: text("objective").notNull(), depth: integer("depth").notNull(), status: delegationStatus("status").notNull().default("ACTIVE"), contextManifestHash: text("context_manifest_hash").notNull(), idempotencyKey: text("idempotency_key").notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), completedAt: timestamp("completed_at", { withTimezone: true, mode: "string" }), revokedAt: timestamp("revoked_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("agent_delegation_child_task_idx").on(table.accountId, table.childTaskId), uniqueIndex("agent_delegation_idempotency_idx").on(table.accountId, table.idempotencyKey), index("agent_delegation_parent_idx").on(table.accountId, table.parentDelegationId), index("agent_delegation_parent_task_idx").on(table.accountId, table.parentTaskId, table.status)]);

export const delegationContextEntries = pgTable("delegation_context_entries", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), delegationId: text("delegation_id").notNull().references(() => agentDelegations.id, { onDelete: "cascade" }), sourceMemoryId: text("source_memory_id").notNull().references(() => memories.id, { onDelete: "restrict" }), classification: dataClassification("classification").notNull(), contentSnapshot: text("content_snapshot").notNull(), contentHash: text("content_hash").notNull(), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("delegation_context_memory_idx").on(table.accountId, table.delegationId, table.sourceMemoryId), index("delegation_context_tenant_idx").on(table.accountId, table.delegationId)]);

export const delegatedAuthorities = pgTable("delegated_authorities", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), delegationId: text("delegation_id").notNull().references(() => agentDelegations.id, { onDelete: "cascade" }), parentLeaseId: text("parent_lease_id").notNull().references(() => capabilityLeases.id, { onDelete: "restrict" }), childLeaseId: text("child_lease_id").references(() => capabilityLeases.id, { onDelete: "restrict" }), capability: jsonb("capability").notNull(), resource: jsonb("resource").notNull(), authorityHash: text("authority_hash").notNull(), maxCalls: integer("max_calls").notNull(), status: delegatedAuthorityStatus("status").notNull().default("AVAILABLE"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(), claimedAt: timestamp("claimed_at", { withTimezone: true, mode: "string" }),
}, (table) => [uniqueIndex("delegated_authority_parent_idx").on(table.accountId, table.delegationId, table.parentLeaseId), uniqueIndex("delegated_authority_child_idx").on(table.childLeaseId), index("delegated_authority_tenant_idx").on(table.accountId, table.delegationId, table.status)]);

export const delegationBudgetAllocations = pgTable("delegation_budget_allocations", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), delegationId: text("delegation_id").notNull().references(() => agentDelegations.id, { onDelete: "cascade" }), parentBudgetId: text("parent_budget_id").notNull().references(() => budgets.id, { onDelete: "restrict" }), childBudgetId: text("child_budget_id").notNull().references(() => budgets.id, { onDelete: "restrict" }), amount: numeric("amount", { precision: 30, scale: 9 }).notNull(), dimension: budgetDimension("dimension").notNull(), currency: text("currency"), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("delegation_budget_parent_idx").on(table.accountId, table.delegationId, table.parentBudgetId), uniqueIndex("delegation_budget_child_idx").on(table.accountId, table.childBudgetId), index("delegation_budget_tenant_idx").on(table.accountId, table.delegationId)]);

export const delegationResults = pgTable("delegation_results", {
  id: text("id").primaryKey(), accountId: text("account_id").notNull().references(() => accounts.id, { onDelete: "cascade" }), delegationId: text("delegation_id").notNull().references(() => agentDelegations.id, { onDelete: "restrict" }), childTaskId: text("child_task_id").notNull().references(() => v2Tasks.id, { onDelete: "restrict" }), outcome: text("outcome").notNull(), result: jsonb("result").notNull(), resultHash: text("result_hash").notNull(), evidenceArtifactIds: text("evidence_artifact_ids").array().notNull().default([]), createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
}, (table) => [uniqueIndex("delegation_result_idx").on(table.accountId, table.delegationId), index("delegation_result_tenant_idx").on(table.accountId, table.childTaskId)]);
