...
async function main(){
  ...
  if(!ready.ready||!config.signer){console.info(JSON.stringify({event:"channel_worker_not_ready",issues:ready.issues,storageAvailable:ready.storageAvailable}));await closeDatabase();return;}
  ...
  while(!stopped){
    try{await runChannelExecutionCycle(config,executor,config.signer);await runChannelDeliveryCycle(config,sender,config.signer);}
